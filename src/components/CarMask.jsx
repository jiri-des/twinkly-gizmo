...
// Car occupies x:77–494, y:86–381 in 568×442 SVG space. Centre x ≈ 285.5.
// State 0 – full car  (~92 % screen width)
// State 1 – left half (zoom 2×, x:60→286)
// State 2 – right half (zoom 2×, x:285→511)
const STATES = [
  { x: 60,  y: 69, w: 451, h: 329 },
  { x: 60,  y: 69, w: 226, h: 329 },
  { x: 285, y: 69, w: 226, h: 329 },
];

// Constant string — React will never diff/update viewBox on its own,
// so direct setAttribute calls during animation are safe.
const INITIAL_VB = `${STATES[0].x} ${STATES[0].y} ${STATES[0].w} ${STATES[0].h}`;
...
export default function CarMask({ stateIdx = 0 }) {
  ...
  useEffect(() => {
    const from = STATES[prevIdxRef.current];
    const to   = STATES[stateIdx];
    ...
    function applyVB({ x, y, w, h }) {
      const s = `${x} ${y} ${w} ${h}`;
      overlaySvgRef.current?.setAttribute('viewBox', s);
      outlineSvgRef.current?.setAttribute('viewBox', s);
    }
  ...
  return (
    <>
      {/* Dark overlay — car area cut out via evenodd */}
      <svg
        ref={overlaySvgRef}
        viewBox={INITIAL_VB}
      ...
      {/* Car outline — inline path, no filter/blend deps */}
      <svg
        ref={outlineSvgRef}
        viewBox={INITIAL_VB}
        preserveAspectRatio="xMidYMid meet"
